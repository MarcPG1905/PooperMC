                        ...
                        .executes(context -> {
                            ...
                            for (Player player : Peelocity.SERVER.getAllPlayers()) {
                                if (player.hasPermission("pee.staff")) {
                                    player.sendMessage(Translation.component(player.getEffectiveLocale(), "staff_chat.message", ((Player) context.getSource()).getUsername(), message).color(NamedTextColor.BLUE));
                                }
                            }
                            return 1;
                        })
